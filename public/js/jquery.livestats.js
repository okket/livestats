var LiveStatsClient;
LiveStatsClient = function() {
  var self;
  if (!this instanceof arguments.callee) {
    return new arguments.callee(arguments);
  }
  self = this;
  this.init = function() {
    self.drawMap();
    self.viewDidResize();
    self.setupBayeuxHandler();
    return this;
  };
  this.setupBayeuxHandler = function() {
    self = this;
    return $.getJSON("/config.json", function(config) {
      self.client = new Faye.Client('http://' + window.location.hostname + ':' + config.port + '/faye', {
        timeout: 120
      });
      return self.client.subscribe('/stat', function(message) {
        return self.drawMarker(message);
      });
    });
  };
  this.viewDidResize = function() {
    var mapCanvasHeight, width, windowHeight;
    self = this;
    width = $('body').width();
    windowHeight = $(window).height();
    mapCanvasHeight = width * (369.0 / 567.0);
    self.map.setSize(width, mapCanvasHeight);
    return $('#map').css({
      'margin-top': (windowHeight - mapCanvasHeight) / 2.0
    });
  };
  this.drawMap = function() {
    self = this;
    self.map = Raphael('map', 0, 0);
    self.map.canvas.setAttribute('viewBox', '0 0 567 369');
    return self.map.path(mapPath).attr({
      stroke: 'black',
      fill: '#222'
    }).attr({
      'stroke-width': 0.7
    });
  };
  this.geoCoordsToMapCoords = function(latitude, longitude) {
    var mapHeight, mapOffsetX, mapOffsetY, mapWidth, x, y;
    latitude = parseFloat(latitude);
    longitude = parseFloat(longitude);
    mapWidth = 567;
    mapHeight = 369;
    x = (mapWidth * (180 + longitude) / 360) % mapWidth;
    latitude = latitude * Math.PI / 180;
    y = Math.log(Math.tan((latitude / 2) + (Math.PI / 4)));
    y = (mapHeight / 2) - (mapWidth * y / (2 * Math.PI));
    mapOffsetX = mapWidth * 0.026;
    mapOffsetY = mapHeight * 0.141;
    return {
      x: (x - mapOffsetX) * 0.97,
      y: y + mapOffsetY + 15,
      xRaw: x,
      yRaw: y
    };
  };
  this.drawMarker = function(message) {
    var city, hideFunc, hoverFunc, latitude, longitude, mapCoords, person, subtitle, text, title, x, y;
    self = this;
    latitude = message.latitude;
    longitude = message.longitude;
    text = message.title;
    city = message.city;
    mapCoords = this.geoCoordsToMapCoords(latitude, longitude);
    x = mapCoords.x;
    y = mapCoords.y;
    person = self.map.path(personPath);
    person.scale(0.01, 0.01);
    person.translate(-255, -255);
    person.translate(x, y);
    person.attr({
      fill: '#ff9',
      stroke: 'transparent'
    });
    title = self.map.text(x, y + 11, text);
    title.attr({
      fill: 'white',
      "font-size": 10,
      "font-family": "'Helvetica Neue', 'Helvetica', sans-serif",
      'font-weight': 'bold'
    });
    subtitle = self.map.text(x, y + 21, city);
    subtitle.attr({
      fill: '#999',
      "font-size": 7,
      "font-family": "'Helvetica Neue', 'Helvetica', sans-serif"
    });
    hoverFunc = function() {
      person.attr({
        fill: 'white'
      });
      $(title.node).fadeIn('fast');
      return $(subtitle.node).fadeIn('fast');
    };
    hideFunc = function() {
      person.attr({
        fill: '#ff9'
      });
      $(title.node).fadeOut('slow');
      return $(subtitle.node).fadeOut('slow');
    };
    $(person.node).hover(hoverFunc, hideFunc);
    return person.animate({
      scale: '0.02, 0.02'
    }, 2000, 'elastic', function() {
      $(title.node).fadeOut(5000);
      return $(subtitle.node).fadeOut(5000);
    });
  };
  return this.init();
};
jQuery(function() {
  var liveStatsClient;
  liveStatsClient = new LiveStatsClient();
  return $(window).resize(function() {
    return liveStatsClient.viewDidResize();
  });
});